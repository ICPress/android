...
public class CropOverlayView extends View {
  ...
  private Paint mBorderCornerPaintDark;
  ...
  private boolean mIsTitleBackgroundImage;
  ...
  public void setIsTitleBackgroundImage(boolean isTitleBackgroundImage) {
    mIsTitleBackgroundImage =  isTitleBackgroundImage;
  }

  public boolean isTitleBackgroundImage() {
    return mIsTitleBackgroundImage;
  }
  ...
  public void setInitialAttributeValues(CropImageOptions options) {
    ...
    mBorderCornerPaintDark =
            getNewPaintOrNull(options.borderCornerThickness,0xAF808084);
    ...
    mIsTitleBackgroundImage = options.isTitleBackgroundImage;
  }
  ...
  private void drawCorners(Canvas canvas) {
    if (mBorderCornerPaint != null) {
      ...
      int baseCornerOffset = 0;
      int leftRightOffset;
      if (isFixAspectRatio() || isTitleBackgroundImage()) leftRightOffset = 55;
      else leftRightOffset = 0;
      if (mCropShape== CropImageView.CropShape.RECTANGLE) {
        // Top left
        canvas.drawLine(
                rect.left + leftRightOffset - cornerOffset + baseCornerOffset,
                rect.top - cornerExtension + baseCornerOffset,
                rect.left + leftRightOffset - cornerOffset + baseCornerOffset,
                rect.top + mBorderCornerLength + baseCornerOffset + 5,
                mBorderCornerPaintDark);
        canvas.drawLine(
                rect.left + leftRightOffset + 5 - cornerOffset + baseCornerOffset,
                rect.top + 5 - cornerExtension + baseCornerOffset,
                rect.left + leftRightOffset + 5 - cornerOffset + baseCornerOffset,
                rect.top + mBorderCornerLength + 5 + baseCornerOffset,
                mBorderCornerPaint);
        canvas.drawLine(
                rect.left + leftRightOffset - cornerExtension + baseCornerOffset,
                rect.top - cornerOffset + baseCornerOffset,
                rect.left + leftRightOffset + mBorderCornerLength + baseCornerOffset + 5,
                rect.top - cornerOffset + baseCornerOffset,
                mBorderCornerPaintDark);
        canvas.drawLine(
                rect.left + leftRightOffset + 5 - cornerExtension + baseCornerOffset,
                rect.top + 5 - cornerOffset + baseCornerOffset,
                rect.left + leftRightOffset + mBorderCornerLength + 5 + baseCornerOffset,
                rect.top + 5 - cornerOffset + baseCornerOffset,
                mBorderCornerPaint);

        // Top right
        canvas.drawLine(
                rect.right - leftRightOffset + 5 + cornerOffset - baseCornerOffset,
                rect.top - cornerExtension + baseCornerOffset,
                rect.right - leftRightOffset + 5 + cornerOffset - baseCornerOffset,
                rect.top + mBorderCornerLength + baseCornerOffset + 5,
                mBorderCornerPaintDark);
        canvas.drawLine(
                rect.right - leftRightOffset + cornerOffset - baseCornerOffset,
                rect.top + 5 - cornerExtension + baseCornerOffset,
                rect.right - leftRightOffset + cornerOffset - baseCornerOffset,
                rect.top + 5 + mBorderCornerLength + baseCornerOffset,
                mBorderCornerPaint);
        canvas.drawLine(
                rect.right - leftRightOffset + 5 + cornerExtension - baseCornerOffset,
                rect.top - cornerOffset + baseCornerOffset,
                rect.right - leftRightOffset - mBorderCornerLength - baseCornerOffset,
                rect.top - cornerOffset + baseCornerOffset,
                mBorderCornerPaintDark);

        canvas.drawLine(
                rect.right - leftRightOffset + cornerExtension - baseCornerOffset,
                rect.top + 5 - cornerOffset + baseCornerOffset,
                rect.right - leftRightOffset - mBorderCornerLength - baseCornerOffset,
                rect.top + 5 - cornerOffset + baseCornerOffset,
                mBorderCornerPaint);
        ...
        canvas.drawLine(
                rect.left + leftRightOffset - cornerOffset + baseCornerOffset,
                rect.bottom + 5 + cornerExtension - baseCornerOffset,
                rect.left + leftRightOffset - cornerOffset + baseCornerOffset,
                rect.bottom - mBorderCornerLength - baseCornerOffset,
                mBorderCornerPaintDark);
        canvas.drawLine(
                rect.left + leftRightOffset + 5 - cornerOffset + baseCornerOffset,
                rect.bottom + cornerExtension - baseCornerOffset,
                rect.left + leftRightOffset + 5 - cornerOffset + baseCornerOffset,
                rect.bottom - mBorderCornerLength - baseCornerOffset,
                mBorderCornerPaint);
        canvas.drawLine(
                rect.left + leftRightOffset - cornerExtension + baseCornerOffset,
                rect.bottom + 5 + cornerOffset - baseCornerOffset,
                rect.left + 5 + leftRightOffset + mBorderCornerLength + baseCornerOffset,
                rect.bottom + 5 + cornerOffset - baseCornerOffset,
                mBorderCornerPaintDark);
        canvas.drawLine(
                rect.left + leftRightOffset + 5 - cornerExtension + baseCornerOffset,
                rect.bottom + cornerOffset - baseCornerOffset,
                rect.left + leftRightOffset + 5 + mBorderCornerLength + baseCornerOffset,
                rect.bottom + cornerOffset - baseCornerOffset,
                mBorderCornerPaint);
